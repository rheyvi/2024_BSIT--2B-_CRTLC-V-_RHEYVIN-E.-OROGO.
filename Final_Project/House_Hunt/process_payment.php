...
if (isset($_POST['payment_method'])) {
    ...
    // Update house status to 'Sold' in the houses table
    $update_house_query = "UPDATE houses SET Status = 'Sold' WHERE houseId = '$house_id'";
    $execute_update_house = mysqli_query($conn, $update_house_query);

    if (!$execute_update_house) {
        die("Error updating house status: " . mysqli_error($conn));
    }
    ...
    if ($execute_insert_payment) {
        echo '<script>alert("Order Successfully"); window.location.href = "cart.php";</script>';
    } else {
